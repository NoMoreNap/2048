...
let fxProps = {
    count: 5,
    interval: 2000,
    colors: ['#cc3333', '#4CAF50', '#81C784','rgba(255, 175, 0, 1)','rgba(255, 14, 65, 1)'],
...
export const MaxScore: React.FC<IModalContent> = ({handleClose}) => {
    const result = useGlobalValue(RESULT)
    const [isFireworks, setIsFireWorks] = React.useState(true)
    ...
    React.useEffect(() => {
        setTimeout(() => setIsFireWorks(false), 1500)
    },[])
    return (
        <Box sx={{display: 'flex', alignItems: 'center', position: 'relative', flexDirection: 'column', justifyContent: 'center', gap: 10,    p: 4, overflow: 'visible'}}>
            ...
            <Box sx={{marginTop: '50px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                <Typography sx={{fontSize: '2rem', fontWeight: 500, color: 'rgba(71, 67, 250, 1)'}}>Вы побили свой рекорд!</Typography>
